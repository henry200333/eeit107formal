...
public class PerformanceDao {
	...
	private static final String INSERT = "INSERT INTO Performance(a_id, p_name, p_url, update_time,counts) VALUES(?,?,?,GETDATE(),?)";
	private static final String UPDATE = "UPDATE Performance SET a_id=?, p_name=?, p_url=?, update_time=GETDATE(), counts=? WHERE p_id = ?";
	...
	public void insert(Performance performance) {
		try {
			...
			pstmt.setInt(4, performance.getCounts());
			pstmt.executeUpdate();
	...
	public void update(Performance performance) {
		try {
			...
			pstmt.setString(3, performance.getpUrl());
			pstmt.setInt(4, performance.getCounts());
			pstmt.setInt(5, performance.getpId());
			pstmt.executeUpdate();
		} catch (SQLException e) {
			e.printStackTrace();
		}
